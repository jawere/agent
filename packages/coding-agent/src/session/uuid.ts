// UUIDv7 implementation (monotonic, time-ordered)
// Based on RFC 9562 §5.7
...
/**
 * Generate a short unique entry ID (8 hex chars, compatible with pi JSONL format).
 * Falls back to full UUID if collisions detected.
 */
export function generateEntryId(existing: Set<string> | Map<string, unknown>): string {
  for (let i = 0; i < 100; i++) {
