...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @jawere/ai <command> [provider]
...
Examples:
  npx @jawere/ai login              # interactive provider selection
  npx @jawere/ai login anthropic    # login to specific provider
  npx @jawere/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @jawere/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @jawere/ai --help' for usage`);
	process.exit(1);
}
