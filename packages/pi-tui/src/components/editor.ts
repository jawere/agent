...
export interface EditorTheme {
	...
	/**
	 * Cursor highlight style. Override to change cursor appearance.
	 * Default is reverse video (\x1b[7m ... \x1b[0m).
	 * Return an array of [open, close] ANSI escape sequences.
	 * Example for dimmed: ['\x1b[2;7m', '\x1b[0m']
	 * Example for underline: ['\x1b[4m', '\x1b[0m']
	 */
	cursorStyle?: () => [string, string];
}
...
export class Editor implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		// Resolve cursor style: default is reverse video, overridable via theme
		const cursorStyle = this.theme.cursorStyle?.() ?? ["\x1b[7m", "\x1b[0m"];
		const [cursorOpen, cursorClose] = cursorStyle;
		...
		for (const layoutLine of visibleLines) {
			...
			if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
				...
				if (after.length > 0) {
					...
					const cursor = `${cursorOpen}${firstGrapheme}${cursorClose}`;
					displayText = before + marker + cursor + restAfter;
					// lineVisibleWidth stays the same - we're replacing, not adding
				} else {
					// Cursor is at the end - add highlighted space
					const cursor = `${cursorOpen} ${cursorClose}`;
