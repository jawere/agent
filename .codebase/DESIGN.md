...
## Working Memory System (state.md)

### Problem

The agent's second-biggest weakness is lack of internal state/memory. Every tool result is treated as if the agent is seeing the file for the first time, leading to massive over-reading and re-verification. Within a single conversation, the agent may read the same file 3-5 times because it doesn't "remember" what it already read.

### Solution

A working memory file at `.codebase/state.md` serves as the agent's scratchpad between turns. The agent is instructed (via system prompt) to:

1. **Read state.md at the start of every turn** — recall what files have already been read and what the current task is
2. **Write to state.md after significant actions** — record file reads, edits, and progress
3. **Never re-read a file unnecessarily** — if state.md says the file was already read and hasn't changed, skip it

```
┌─────────────────────────────────────────────────────────────┐
│  Agent Turn 1: User says "Fix the bug in auth.ts"          │
│                                                             │
│  1. Read .codebase/state.md → empty, no prior context      │
│  2. Read .codebase/tree.yaml → find auth.ts location       │
│  3. Read src/auth.ts → understand the code                 │
│  4. Edit src/auth.ts → fix the bug                         │
│  5. Write .codebase/state.md → record:                     │
│     - Files Read: src/auth.ts (hash: abc123)               │
│     - Files Modified: src/auth.ts                          │
│     - Current Task: Fixed auth bug, verify compilation     │
│                                                             │
│  Agent Turn 2: User says "Also fix the login flow"        │
│                                                             │
│  1. Read .codebase/state.md → already read auth.ts,        │
│     tree.yaml already loaded, knows about auth.ts          │
│  2. Read src/login.ts → new file, add to state.md          │
│  3. Edit src/login.ts → fix                                │
│  4. Write .codebase/state.md → update notes                │
│                                                             │
│  SAVED: 3-4 turns of re-reading auth.ts + tree.yaml!      │
└─────────────────────────────────────────────────────────────┘
```
...
Session started: 2026-07-03T21:00:00.000Z

## Files Read (with hashes)
- src/auth.ts — hash: abc123 — auth middleware, exports: authenticate, authorize
- .codebase/tree.yaml — project structure loaded

## Files Modified
- src/auth.ts — fixed token expiry check (line 45)

## Current Task
Fixed auth bug, now working on login flow refactor

## Notes
- The bug was in the JWT expiry comparison using > instead of <
- Remember: the config is loaded from ~/.jawere/config.json
```

### Benefits

- **Eliminates redundant reads**: Agent reads each file once per session, not 3-5 times
- **Cross-turn context**: Agent "remembers" what it was doing even after long tool executions
- **Self-documenting**: state.md doubles as a session log showing what was changed
- **Zero new infrastructure**: Just a markdown file + system prompt instructions

---

## Checksum System (checksums.json)

### Problem

Even with tree.yaml, the agent can't tell if a file has been externally modified since it last read it. This forces defensive re-reading.

### Solution

`.codebase/checksums.json` stores SHA-256 content hashes (first 64KB) for every tracked file, generated by the scanner. The agent can:

1. **Check if a file changed** by comparing stored hash vs current hash
2. **Skip re-reading unchanged files** — if the hash matches, the content is the same
3. **Detect external modifications** — if someone edits a file outside the agent, the hash mismatch triggers a re-read

### Format

```json
{
  "scannedAt": 1783129579967,
  "gitHash": "4fd07cc...",
  "files": {
    "src/auth.ts": {
      "hash": "bf98274652d39b92",
      "size": 2145,
      "scannedAt": 1783129579968
    },
    ...
  }
}
...
## Prompt Box Extraction (src/prompt.ts)

### Problem

The multiline prompt with Shift+Enter, bracketed paste, and arrow key support was ~180 lines of raw terminal escape code inline in `src/index.ts`. This made index.ts brittle — the agent could only verify compilation, not runtime TUI behavior. Every edit risked cascading terminal bugs.

### Solution

The prompt box was extracted into its own module (`src/prompt.ts`) with a clean factory API:
...
This isolates complex terminal handling so:
- **index.ts is simpler** — just calls `prompt()` and handles commands
- **prompt.ts is testable in isolation** — can be mocked for tests
- **Future replacements are easy** — swap to a library like ink/ink without touching the main loop
- **Fewer cascading bugs** — changes to prompt logic can't break the main REPL loop
